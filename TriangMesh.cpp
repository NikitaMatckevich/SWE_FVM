...
TriangMesh::TriangMesh(string const& filename) {
  ...
  auto check_tag_format_too_small_to_handle = [&](index i) {
    if (i >= numeric_limits<index>::max())
      throw MeshError("index is too small to handle in " + filename);
  };
  ...
  index num_entity_points, num_entity_curves;
  ...
  unordered_map<index, index> boundary_types;
  while (num_entity_curves-- > 0) {
    index tag;
    double min_x, min_y, min_z, max_x, max_y, max_z;
    index num_phys_tags, boundary_type;
    msh >> tag >>
      min_x >> min_y >> min_z >>
      max_x >> max_y >> max_z >>
      num_phys_tags;
  ...
  find_section("$Nodes");
  index num_entity_blocks, num_nodes;
  ...
  for (index block = 1; block <= num_entity_blocks; block++) {
    index entity_tag, entity_dim, parametric, nodes_in_block;
    msh >> entity_tag >> entity_dim >> parametric >> nodes_in_block;
    for (index node = 1; node <= nodes_in_block; node++) {
      index ind;
      double x, y;
      msh >> ind >> x >> y;
      if (ind != current_node++)
...
void info(ostream& out, TriangMesh const& M) {
  ...
  auto write_comment = bind(
    [&](ostream& out, index wid, string_view c) {out.width(wid); out << c << ' '; },
