...
Array<3> TimeDisc::RHS(Idx i, double dt) const {
  ...
  for (int k = 0; k < 3; k++) {
    ...
    res.tail<2>() += dtk * (b.Norm(ie[k], i) * c_ek * (0.5 * h_ek * h_ek)).array();
  }
...
double TimeDisc::ComputeDrainingDt(Idx i) const {
  ...
  return sum > tol ? 
    b.Area(i) * m_sd->GetVolField().h(i) / sum :
