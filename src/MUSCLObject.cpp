...
MUSCLObject::MUSCL MUSCLObject::ReconstructDryCell(Idx i) const {
    ...
    grad.row(0) = b.Gradient(i).matrix().transpose();
...
MUSCLObject::MUSCL MUSCLObject::ReconstructFullWetCell(Idx i) const {
	...
	Eigen::Matrix23d dx = (b.P(ip) - b.T(i).replicate<1, 3>()).matrix();
	Array<3> wp = (df.row(0) * dx).array() + m_vol.w(i);
	Array<3> hp = wp - b.P(ip).transpose();
	...
	for (int k = 0; k < 3; k++) {
		...
		Array<3> vek   = m_vol.prim(i) + (df * (b.E(ie[k]) - b.T(i)).matrix()).array();
...
MUSCLObject::MUSCL MUSCLObject::ReconstructPartWetCell1(Idx i) const {
	...
	double b13 = b.P(ip).maxCoeff();
	double b23 = b.P(ip).minCoeff();
    ...
    return MUSCL{GetDomain(), Array<3>{w_rec, m_vol.u(i), m_vol.v(i)}, Eigen::Matrix32d::Zero(), i};
}
...
MUSCLObject::MUSCL MUSCLObject::ReconstructPartWetCell2(Idx i) const {
    ...
    grad_values[0] = w23;
    grad_points.row(0) = b.P(ip[0]);
   
    if (hi <= h_delimiter1) { // Case 1
        double k2 = sqrt(3. * ratio_h / ratio_b);
        grad_values    [1] = k2 * b.P(ip[1]) + (1. - k2) * b.P(ip[0]);
        grad_points.row(1) = k2 * b.P(ip[1]) + (1. - k2) * b.P(ip[0]);
        double k3 = sqrt(3. * ratio_h * ratio_b);
        grad_values    [2] = k3 * b.P(ip[2]) + (1. - k3) * b.P(ip[0]);
        grad_points.row(2) = k3 * b.P(ip[2]) + (1. - k3) * b.P(ip[0]); 
    } else if (hi >= h_delimiter2) { // Case 3
        double delta_w = 1.5 * (hi - h_delimiter2);
        grad_values[1] = delta_w + b.P(ip[1]) + (1. - ratio_b) * h23;
        grad_points.row(1) = b.P(ip[1]);
        grad_values[2] = delta_w + b.P(ip[2]);
        grad_points.row(2) = b.P(ip[2]);
    } else { // Case 2
        ...
        double bp1 = k1 * b.P(ip[2]) + (1. - k1) * b.P(ip[1]);
        double bp3 = k3 * b.P(ip[2]) + (1. - k3) * b.P(ip[0]);
        grad_values    [1] = b.P(ip[1]) + (k1 / k3) * beta * h23;
        grad_points.row(1) = b.P(ip[1]);
        grad_values    [2] = bp1;
        grad_points.row(2) = k1 * b.P(ip[2]) + (1. - k1) * b.P(ip[1]);
    }
 
    Eigen::Matrix32d grad = Eigen::Matrix32d::Zero();
    grad.row(0) = grad_values * GradientCoefs(grad_points);
    double wt = w23 + grad.row(0) * (b.T(i) - b.P(ip[0])).matrix();
